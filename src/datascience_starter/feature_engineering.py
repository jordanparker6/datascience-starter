"""A module for feature engineering transformations.

This module contains transformations following the
scikit-learn API for feature engineering.
...
class TimeseriesFeatures(base.TransformerMixin):
    ...
    def __init__(self, alpha: float):
        ...
        self._months = ['jan', 'feb', 'mar', 'apr', 'may','jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'] #: An array of month strings.
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        data = { k: self._rbf(X.index.month, i, self.alpha) for i, k in enumerate(self._months) }
        data = pd.DataFrame(data, index=X.index)
        return pd.concat([data, X], axis=1)

class NumericFeatures(base.TransformerMixin):
    """Transforms the numeric features in a dataframe by a list of functions.
    ...
    """
    def __init__(self):
        self._functions = [lambda x: np.power(x, 2)]
